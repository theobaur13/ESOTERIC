...
from legacy.FEVERISH_3_3.utils.document_retrieval import title_match_search, score_docs, text_match_search, extract_focals, extract_questions, calculate_answerability_score_SelfCheckGPT, calculate_answerability_score_tiny, extract_answers, extract_questions, extract_polar_questions
from legacy.FEVERISH_3_3.utils.NER import extract_entities
...
class EvidenceRetriever:
    def __init__(self, data_path, title_match_docs_limit=1000, text_match_search_db_limit=1000, text_match_search_k_limit=15, title_match_search_threshold=0, text_match_search_threshold=0, answerability_threshold=0.01):
        ...
        relevance_classification_model_dir = os.path.join(self.data_path, '..', 'model')
    ...
    def retrieve_passages(self, evidence_wrapper):
        ...
        for evidence in evidences:
            ...
            for sentence in sentences:
                ...
                if label == "LABEL_1":
                    evidence_sentence = Sentence(sentence=sentence, score=relevance_score, doc_id=evidence.doc_id)
                    evidence_sentences.append(evidence_sentence)

            for sentence in evidence_sentences:
                sentence.set_start_end(text)
